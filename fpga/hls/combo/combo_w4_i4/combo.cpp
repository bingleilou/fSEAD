...
void combo_weight_i4(
    ...
    data_IN4 = IN4.read();
    score_in[3] = (interface_t)data_IN4.data;

    accum_t combo_regression = (accum_t)(((accum_t)score_in[0]*(accum_t)w1 + (accum_t)score_in[1]*(accum_t)w2 + (accum_t)score_in[2]*(accum_t)w3 + (accum_t)score_in[3]*(accum_t)w4)*(accum_t)0.25);
    data_OUT.data = (float)combo_regression;
    data_OUT.last = (data_IN1.last)|(data_IN2.last)|(data_IN3.last)|(data_IN4.last);
    OUT << data_OUT;
