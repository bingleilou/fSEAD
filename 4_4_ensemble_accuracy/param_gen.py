...
def print_params_loda(data_path, data_name, ensemble, dim, batch_size, n_Bins, min_X, projections, delta, bottom_loda, delta_loda):
    f = open(data_path + data_name, "w")
    f.write("#define ENSEMBLE " + str(ensemble) + "\n")
    f.write("#define DIM " + str(dim) + "\n")
...
def print_params_rshash(data_path, ensemble, dim, batch_size, num_hash, matrix_col, alpha_selected, f_selected, min_X, delta_X, bottom_rshash, delta_rshash):
    f = open(data_path + "params_rshash.h", "w")
    f.write("#define ENSEMBLE " + str(ensemble) + "\n")
    f.write("#define DIM " + str(dim) + "\n")
    f.write("#define BATCH_SIZE " + str(batch_size) + "\n")
...
def print_params_xstream(data_path, ensemble, dim, batch_size, K, depth, matrix_col, R_T, fs, deltamax, shift_deltamax, shift, bottom_xstream, delta_xstream):
    f = open(data_path + "params_xstream.h", "w")
    f.write("#define ENSEMBLE " + str(ensemble) + "\n")
    f.write("#define DIM " + str(dim) + "\n")
    f.write("#define BATCH_SIZE " + str(batch_size) + "\n")
