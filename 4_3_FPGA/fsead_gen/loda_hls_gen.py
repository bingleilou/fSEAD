...
for m in range(M):
    hls_path = "./0fsead_0/{ad}_e35/{data}/C{c}/".format(ad=ad,data=data,c=m+1)
    ####################################################################
...
'''
    f.write(s)
    f.write('    rval = write_file_1D<float, SMAPLE_NUM>("D:/HLS/0fsead_0/loda_e35/'+str(data)+'/C'+str(m+1)+'/data/score_loda_'+str(data)+'\", tb_score_loda);\n')
    f.write('}')
...
hls:
	vivado_hls hls_prj.tcl -f
clean:
	rm -rf *.log *.jou .Xil hls_loda_*_prj/ vivado_loda_*_prj/
    '''
    f.write(s)