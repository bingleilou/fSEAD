...
for m in range(M):
    hls_path = "./0fsead_0/{ad}_e20/{data}/C{c}/".format(ad=ad,data=data,c=m+1)
    ####################################################################
...
'''
    f.write(s)
    f.write('    rval = write_file_1D<float, SMAPLE_NUM>("D:/HLS/0fsead_0/xstream_e20/'+str(data)+'/C'+str(m+1)+'/data/score_xstream_'+str(data)+'\", tb_score_xstream);\n')
    f.write('}')
