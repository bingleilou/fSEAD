...
param_path = "./params/4.18/{ad}/{data}/".format(ad=ad,data=data)

M = 1

for m in range(M):
    hls_path = "./0fsead_2/{ad}_e20/{data}/C{c}/".format(ad=ad,data=data,c=m+1)
    ####################################################################
...
'''
    f.write(s)
    f.write('rval = write_file_1D<float, SMAPLE_NUM>("D:/usyd/doctor/HLS/projects/0fsead_2/xstream_e20/'+str(data)+'/C'+str(m+1)+'/data/score_xstream_'+str(data)+'\", tb_score_xstream);\n')
    f.write('}')
...
struct DataBus_OUT {
    ...
    f_params = open(param_path + 'params_xstream'+'_E'+str(ensemble)+'.h', "r")
    ss = f_params.read()
    f.write(ss)
...
hls:
	vivado_hls hls_prj.tcl -f
clean:
	rm -rf *.log *.jou .Xil hls_xstream_*_prj/ vivado_xstream_*_prj/
    '''
    f.write(s)